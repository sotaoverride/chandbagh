...
#warning spi/spi_slave example requires a board with SPI pins
    ...
    uint8_t out_buf[BUF_LEN] = {0xAB, 0xCA, 0xAF}, in_buf[BUF_LEN]={0xfA,0x4D,0x5C}, fw_resp[BUF_LEN-1]={ 0xD5, 0xD5};
	...
	if (in_buf[0] == FWRequest){ spi_write_read_blocking (spi_default, fw_resp, in_buf+1, 2);}
